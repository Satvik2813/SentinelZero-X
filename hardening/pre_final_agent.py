...
def _normalize(task):
    ...
    for i, line in enumerate(lines):
        ...
        if match and match[1].lower() in aliases:
            ...
            parsed.setdefault(key, match[2].strip())
            last = key
...
def solve(task: dict[str, Any], tools: ToolsClient, api_key=None, model=None, base_url=None) -> dict[str, Any]:
    ...
    if conflict:
        ...
        resolution, classification = 'warn', 'suspicious_unknown'
    elif internal and not inv.failures:
        resolution, classification = 'allow', 'internal_legitimate'
    elif (reputation in ('safe', 'trusted') or domain in partners or (established and authenticated)) and not inv.failures:
